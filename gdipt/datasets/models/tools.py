...
from gdipt.datasets.tools import make_ndvi_difference_dataset
...
from gdipt.geoprocessing.utils import gdf_ndvi_validate_years_as_ints
from gdipt.io import save_as_csv, save_as_gpkg, save_as_shp
from gdipt.utils import _WRITE_LOCK, GDIPT_TMP_DIR
...
def build_ndvi_difference_dataset(config: dict[str, Any]):
    ...
    if isinstance(shapefile, pd.DataFrame) or isinstance(shapefile, gpd.GeoDataFrame):
        ...
        source_path = (
            GDIPT_TMP_DIR / f"{TMP_FILE_PREFIX}ndvi_difference_dataset_{timestamp}.shp"
        )
