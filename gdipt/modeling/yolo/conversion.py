...
import pandas as pd
from gdipt.datasets.utils import TMP_FILE_PREFIX
from gdipt.geometry import stringify_points
from gdipt.modeling.utils import (AnnotatedLabel, BBox,
                                  convert_segmented_to_bbox_annotation,
                                  parse_images_from_labels,
                                  parse_labels_from_dataframe)
from gdipt.modeling.yolo import YOLODatasetBase
from gdipt.utils import GDIPT_TMP_DIR, NUM_CPU
...
def geodataframe_to_yolo(
    ...
    tmp_path = GDIPT_TMP_DIR / f"{TMP_FILE_PREFIX}yolo_ds_{time()}.csv"
    gdf.to_csv(tmp_path)
    num_workers = NUM_CPU
